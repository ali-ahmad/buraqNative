...
export function useAudioHelper(request: IUseAudioHelper = {
    ...
    const [player, setPlayer] = React.useState<SoundPlayer>({} as SoundPlayer);
    ...
    function initialize() {
        ...
        if (listSounds.length > 0) {
            ...
            const callback = (error: { message: React.SetStateAction<string>; }, player: SoundPlayer) => {
                if (error) {
                    setStatus('error');
    ...
    function playList(id:number) {
        ...
        //         let newRemainingIndices = shuffleArray(remainingIndices.length === 0 ? [...Array(request.listSounds.length).keys()].filter(value => value !== index) : remainingIndices);
        //         setRemainingIndices(newRemainingIndices);
        //         setIndex(newRemainingIndices[1]);
        //     } else {
        //         setIndex((index + id) % request.listSounds.length);
        //     }
        // }
       if(id === index) {
        player.pause();
